...
function AdminPanel:ShowCharacterDetails(member, info, hasAddon, isEnabled)
    ...
    -- Security Info button (gold-styled)
    local infoBtn = CreateGoldButton(content, 220, 26, "Security-Info anfordern")
    infoBtn:SetPoint("TOPLEFT", 0, yOffset)
    infoBtn:SetScript("OnClick", function()
        local targetName = member.shortName or member.displayName
        if targetName then
            BR:SendAddonMessage("REQUEST_SECURITY_INFO:" .. targetName, "GUILD")
            BR:Print("Security-Info von " .. targetName .. " angefordert.", "info")
        end
    end)
    infoBtn:SetScript("OnEnter", function(self)
        ...
        self.text:SetTextColor(1, 0.9, 0.6, 1)
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText("Security-Info anfordern", 1, 1, 1)
        GameTooltip:AddLine("Fordert SecurityCheck-Daten vom Spieler an.", 0.8, 0.8, 0.8, true)
        GameTooltip:AddLine("(/played, gespeicherte Zeit, Differenz)", 0.6, 0.6, 0.6, true)
        GameTooltip:Show()
    end)
    ...
    -- Update Request button (gold-styled)
    local updateBtn = CreateGoldButton(content, 220, 26, "Update-Hinweis senden")
    updateBtn:SetPoint("TOPLEFT", 0, yOffset)
    updateBtn:SetScript("OnClick", function()
        local targetName = member.displayName or member.shortName
        if targetName and member.isOnline then
            SendChatMessage("Bitte aktualisiere das AllforOne Addon auf die neueste Version. Danke!", "WHISPER", nil, targetName)
            BR:Print("Update-Hinweis an " .. targetName .. " gesendet.", "info")
        elseif not member.isOnline then
            BR:Print("Spieler ist offline - Whisper nicht möglich.", "warning")
        end
    end)
    updateBtn:SetScript("OnEnter", function(self)
        ...
        self.text:SetTextColor(1, 0.9, 0.6, 1)
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText("Update-Hinweis senden", 1, 1, 1)
        GameTooltip:AddLine("Sendet dem Spieler eine Aufforderung,", 0.8, 0.8, 0.8, true)
        GameTooltip:AddLine("das Addon auf die neueste Version zu aktualisieren.", 0.8, 0.8, 0.8, true)
        GameTooltip:Show()
    end)
    ...
    -- Unlock Flying button (gold-styled)
    local unlockFlyingBtn = CreateGoldButton(content, 105, 26, "Fliegen: AN")
    unlockFlyingBtn:SetPoint("TOPLEFT", 0, yOffset)
    unlockFlyingBtn:SetScript("OnClick", function()
        local targetName = member.shortName or member.displayName
        if targetName then
            BR:SendAddonMessage("UNLOCK_FLYING:" .. targetName, "GUILD")
            BR:Print("Flug-Freischaltung an " .. targetName .. " gesendet.", "info")
        end
    end)
    unlockFlyingBtn:SetScript("OnEnter", function(self)
        self:SetBackdropColor(0.25, 0.2, 0.12, 1)
        self:SetBackdropBorderColor(1, 0.9, 0.6, 1)
        self.text:SetTextColor(1, 0.9, 0.6, 1)
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText("Fliegen freischalten", 1, 1, 1)
        GameTooltip:AddLine("Schaltet Himmelsreiten für diesen Spieler", 0.8, 0.8, 0.8, true)
        GameTooltip:AddLine("temporär frei (nur bis zum nächsten Login).", 0.8, 0.8, 0.8, true)
        GameTooltip:Show()
    end)
    unlockFlyingBtn:SetScript("OnLeave", function(self)
        self:SetBackdropColor(0.15, 0.12, 0.08, 0.95)
        self:SetBackdropBorderColor(0.796, 0.71, 0.482, 1)
        self.text:SetTextColor(0.796, 0.71, 0.482, 1)
        GameTooltip:Hide()
    ...
    -- Lock Flying button (gold-styled)
    local lockFlyingBtn = CreateGoldButton(content, 105, 26, "Fliegen: AUS")
    lockFlyingBtn:SetPoint("TOPLEFT", 115, yOffset)
    lockFlyingBtn:SetScript("OnClick", function()
        local targetName = member.shortName or member.displayName
        if targetName then
            BR:SendAddonMessage("LOCK_FLYING:" .. targetName, "GUILD")
            BR:Print("Flug-Sperre an " .. targetName .. " gesendet.", "info")
        end
    end)
    lockFlyingBtn:SetScript("OnEnter", function(self)
        self:SetBackdropColor(0.25, 0.2, 0.12, 1)
        self:SetBackdropBorderColor(1, 0.9, 0.6, 1)
        self.text:SetTextColor(1, 0.9, 0.6, 1)
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText("Fliegen sperren", 1, 1, 1)
        GameTooltip:AddLine("Setzt die temporäre Freischaltung zurück.", 0.8, 0.8, 0.8, true)
        GameTooltip:AddLine("Flugsperre wird wieder aktiv.", 0.8, 0.8, 0.8, true)
        GameTooltip:Show()
    end)
    lockFlyingBtn:SetScript("OnLeave", function(self)
        self:SetBackdropColor(0.15, 0.12, 0.08, 0.95)
        self:SetBackdropBorderColor(0.796, 0.71, 0.482, 1)
        self.text:SetTextColor(0.796, 0.71, 0.482, 1)
        GameTooltip:Hide()
